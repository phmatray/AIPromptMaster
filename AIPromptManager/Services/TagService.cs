    ...
    : ITagService
{
    public async Task<IEnumerable<Tag>> GetAllTagsAsync()
    {
        try
        {
            logger.LogDebug("Retrieving all tags");
    ...
    public async Task<IEnumerable<string>> GetTagSuggestionsAsync(string input)
    {
        try
        {
            logger.LogDebug("Getting tag suggestions for input: {Input}", input);
            ...
            var sanitizedInput = validationService.SanitizeInput(input);
        ...
        {
            logger.LogError(ex, "Error getting tag suggestions for input: {Input}", input);
            throw;
        }
    }
    ...
    public async Task<Tag> CreateTagAsync(string name)
    {
        ...
        try
        {
            ...
            var validationResult = validationService.ValidateTagName(name);
            if (!validationResult.IsValid)
            {
                var errorMessage = string.Join("; ", validationResult.Errors);
                logger.LogWarning("Tag validation failed for '{TagName}': {ValidationErrors}", name, errorMessage);
                throw new ArgumentException($"Tag validation failed: {errorMessage}");
            }

            logger.LogDebug("Creating tag: {TagName}", name);
            ...
            if (existingTag != null)
            {
                logger.LogDebug("Tag already exists: {TagName}", name);
                return existingTag;
            }
            ...
            try
            {
                ...
                logger.LogInformation("Successfully created tag: {TagName}", name);
                return newTag;
            }
            catch (DbUpdateException ex) when (IsUniqueConstraintViolation(ex))
            {
                logger.LogWarning(ex, "Unique constraint violation creating tag: {TagName}", name);
            ...
            {
                logger.LogError(ex, "Database constraint violation when creating tag: {TagName}", name);
