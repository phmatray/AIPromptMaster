...
public class ValidationService : IValidationService
{
    ...
    private const int MaxContentLength = 50000; // Reasonable limit for prompt content
    private const int MaxTagsPerPrompt = 20;
    ...
    public ValidationResult ValidatePrompt(Prompt prompt)
    {
        ...
        else
        {
            ...
            {
                result.AddError($"Title cannot exceed {MaxTitleLength} characters");
            }
        ...
        {
            if (prompt.Description.Length > MaxDescriptionLength)
            {
        ...
        {
            if (prompt.Content.Length > MaxContentLength)
            {
                result.AddError($"Content cannot exceed {MaxContentLength} characters");
            }
        ...
        if (prompt.Tags != null)
        {
            if (prompt.Tags.Count > MaxTagsPerPrompt)
            {
                result.AddError($"Cannot have more than {MaxTagsPerPrompt} tags per prompt");
            }
    ...
    public ValidationResult ValidateTagName(string tagName)
    {
        ...
        {
            result.AddError($"Tag name cannot exceed {MaxTagNameLength} characters");
        }
    ...
    public bool IsValidTagName(string tagName)
    {
        return !string.IsNullOrWhiteSpace(tagName) &&
               tagName.Length <= MaxTagNameLength &&
